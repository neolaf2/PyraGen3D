  ...
  return (
    <div className="h-full flex flex-col bg-slate-900/30">
      <div className="p-6 border-b border-slate-800 flex items-center gap-3">
        ...
        <span className="ml-auto bg-slate-800 text-slate-500 text-[10px] px-2 py-0.5 rounded-full font-bold">
          {history.length}
        </span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-3">
        {history.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-slate-600 px-4 text-center">
            ...
            <p className="text-xs uppercase tracking-widest font-bold opacity-40">Architect's Log Empty</p>
          </div>
        ) : (
          history.map(item => (
            <button
              ...
              className="w-full group relative aspect-square rounded-xl overflow-hidden border border-slate-800 hover:border-blue-500 transition-all shadow-lg bg-slate-900"
            >
              <img 
                src={item.imageUrl} 
                alt="History item" 
